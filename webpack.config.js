...
plugins.push(new HtmlWebpackPlugin({  //根据模板插入css/js等生成最终HTML
    filename: __dirname + '/paibandP/paibandP.html', //生成的html存放路径，相对于 path
    template: './paibandP.html', //html模板路径
    inject: true
}));
plugins.push( new HtmlWebpackPlugin({
        template: 'html-withimg-loader!./paibandP.html',
        filename: __dirname + '/paibandP/paibandP.html'
}));

module.exports = {
    watch: true,
    entry: {
        paibandP: './src/paibandP/paibandP.js' //编译的入口文件
    },
    output: {
        publicPath: '/paibandP/', //静态资源访问路径
        path: __dirname + '/paibandP/', //编译到当前目录
