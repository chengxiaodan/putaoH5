...
plugins.push(new HtmlWebpackPlugin({  //根据模板插入css/js等生成最终HTML
    filename: __dirname + '/paibandM/paibandM.html', //生成的html存放路径，相对于 path
    template: './paibandM.html', //html模板路径
    inject: true
}));
plugins.push( new HtmlWebpackPlugin({
        template: 'html-withimg-loader!./paibandM.html',
        filename: __dirname + '/paibandM/paibandM.html'
}));

module.exports = {
    watch: true,
    entry: {
        paibandM: './src/paibandM/paibandM.js' //编译的入口文件
    },
    output: {
        publicPath: '/paibandM/', //静态资源访问路径
        path: __dirname + '/paibandM/', //编译到当前目录
